import { ENTRY_AREAS, EVENT_TYPES, GRADE_AREAS, GRADE_TYPES, OVERRIDING_GRADE_TYPES, PROJECT_STATUS, SCORE_KINDS } from "./constants.js";
...
export function validateScore(score) {
  ...
  if (score.kind === SCORE_KINDS.POINTS_0_TO_15) {
    assert(Number.isInteger(score.value) && score.value >= 0 && score.value <= 15, "points_0_to_15 value must be an integer between 0 and 15");
  }
}
...
export function validateGradeEntry(entry, data) {
  ...
  assertKnownValue(entry.area, ENTRY_AREAS, "grade area");
  assertIsoDate(entry.date, "grade date");
  validateScore(entry.score);
  assert(Array.isArray(entry.topics), "topics must be an array");

  // area "total" only makes sense for an official grade that overrides the
  // subject's overall result directly; regular entries must target oral or
  // written, since "total" is not part of any category weighting.
  if (entry.area === ENTRY_AREAS.TOTAL) {
    assert(
      OVERRIDING_GRADE_TYPES.includes(entry.type),
      "area 'total' is only valid for Zeugnis/Rückmeldung entries"
    );
  }

  if (OVERRIDING_GRADE_TYPES.includes(entry.type) && entry.area !== ENTRY_AREAS.TOTAL) {
    assert(
      typeof entry.overrideCategory === "string" && entry.overrideCategory.length > 0,
      "overrideCategory is required for Zeugnis/Rückmeldung entries in area oral or written"
    );
    const categoryWeights = subject.weights.categories[entry.area] ?? {};
    assert(
      Object.prototype.hasOwnProperty.call(categoryWeights, entry.overrideCategory),
      `overrideCategory must be an existing category for area ${entry.area}: ${entry.overrideCategory}`
    );
  }
}
