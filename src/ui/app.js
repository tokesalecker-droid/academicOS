...
const typeLabels = {
  ...
  [GRADE_TYPES.ZEUGNIS]: "Zeugnis",
  [GRADE_TYPES.RUECKMELDUNG]: "Rückmeldung"
};
    ...
    <form id="grade-form" class="panel form-grid">
      ...
      <label class="field">
        ...
        <select class="select" name="area">
          <option value="${ENTRY_AREAS.ORAL}">Mündlich</option>
          <option value="${ENTRY_AREAS.WRITTEN}">Schriftlich</option>
          <option value="${ENTRY_AREAS.TOTAL}" data-total-option>Gesamt (nur Zeugnis/Rückmeldung)</option>
        </select>
      </label>
      <label class="field" data-override-category-field style="display:none">
        <span class="label">Kategorie (welche Wertung wird ersetzt)</span>
        <select class="select" name="overrideCategory"></select>
      </label>
      <label class="field">
      ...
      <label class="field">
        ...
        <select class="select" name="scoreKind">
          ...
          <option value="${SCORE_KINDS.POINTS_0_TO_15}">1-15 Punkte (Oberstufe)</option>
...
function attachForms() {
  const gradeForm = app.querySelector("#grade-form");
  gradeForm?.addEventListener("submit", handleGradeSubmit);
  if (gradeForm) {
    ["subjectId", "type", "area"].forEach((name) => {
      gradeForm.querySelector(`[name="${name}"]`)?.addEventListener("change", () => updateOverrideCategoryField(gradeForm));
    });
    updateOverrideCategoryField(gradeForm);
  }
...
// Shows/hides and populates the "overrideCategory" field, and disables the
// "total" area option unless the selected type is Zeugnis/Rückmeldung
// (area "total" is only meaningful for an overriding entry).
function updateOverrideCategoryField(gradeForm) {
  const wrapper = gradeForm.querySelector("[data-override-category-field]");
  const select = wrapper.querySelector("select[name=overrideCategory]");
  const typeField = gradeForm.querySelector("[name=type]");
  const areaField = gradeForm.querySelector("[name=area]");
  const type = typeField.value;
  const area = areaField.value;
  const subjectId = gradeForm.querySelector("[name=subjectId]").value;
  const subject = state.data.entities.subjects[subjectId];

  const isOverridingType = OVERRIDING_GRADE_TYPES.includes(type);

  const totalOption = areaField.querySelector("[data-total-option]");
  totalOption.disabled = !isOverridingType;
  if (!isOverridingType && area === ENTRY_AREAS.TOTAL) {
    areaField.value = ENTRY_AREAS.ORAL;
  }

  const needsCategory = isOverridingType && areaField.value !== ENTRY_AREAS.TOTAL;

  if (!needsCategory || !subject) {
  ...
  const categories = Object.keys(subject.weights.categories[areaField.value] ?? {});
...
function handleGradeSubmit(event) {
  ...
  const overrideCategory = OVERRIDING_GRADE_TYPES.includes(type) && area !== ENTRY_AREAS.TOTAL
    ? form.get("overrideCategory")
    : null;
